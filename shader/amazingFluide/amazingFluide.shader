...
uniform bool useTextureForBaseColor = false;
...
void fragment(){
	...
	if (COLOR.a != 0.0){
		...
		float ix = x * circleSize + sin(speed* TIME * 0.1) * 2.0;
		float iy = y * circleSize + cos(speed * TIME * 0.12);
		...
		if (m < 0.1)
			color = mix(c, colorB, alphaB);
		else{
			color = mix(c, colorB, min(alphaB, alphaB * (0.22 * shine/ m2)));
		}
